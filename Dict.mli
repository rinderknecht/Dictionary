(** The module {! Dict} ("dictionary") exports two submodules, {! Std}
    and {! Opt}, whose signatures are those of a dictionary, that is,
    a monotonic mapping from strings to definitions (no deletions).

    The module {! Opt} ("optimised") features dictionaries with faster
    searches than those of module {! Std} ("standard"), but no
    insertions. An optimised dictionary is obtained from a standard
    one when the vocabulary is fixed, and it is always possible to
    revert to the standard version. In other words, a dictionary from
    module {! Opt} can be considered a constant and compact version of
    {! Std}, with fast searches.
*)
...
(** The type {! entry} represents entries in a dictionary of
    definitions of type [def], that is, pairs of a word and a
    definition. *)
...
module rec Std : sig
  ...
  (** The type {! t} denotes a dictionary, that is, a mapping
      from words of type [string] to definitions of type [def]. *)
type 'def t

(** The type {! dict} is an alias to be used when opening the
    module {! Std}. *)
type 'def dict = 'def t
...
(** Let [char] be a character and [dict] a dictionary. If [dict]
    ...
    their first letter}; if there are no such words, the value is the
    empty dictionary. *)
val suffix : char -> 'def t -> 'def t

(** If a dictionary [dict] contains a definition [def] of the word
    [word], then the value of [find word dict] is [def], otherwise the
...
(** The call [add word def dict] evaluates in a dictionary containing
    all the entries of the dictionary [dict], plus the entry [(word,
    def)]. If [word] was already present in [dict], the exception
    [Found word] ("redefinition") is raised, else, if the entry
    [(word, def)] was already present in [dict], then [dict] is
    returned (implying physical equality: [dict == add word def
    dict]). *)
val add : word -> 'def -> 'def t -> 'def t

(** The exception [Found w] is raised when adding the word [w] to a
    dictionary where it is already defined. In other words, there is
    no shadowing of previous entries by default. *)
...
(** Converting to the (space-)optimised representation (see module
    [Opt]). The cross-module invariants are [Std.freeze (Opt.thaw d) =
    d] and [Opt.thaw (Std.freeze d) = d] *)
...
(** (Maintainer) The call [inorder dict] evaluates in the list of
    ...
    corresponding entries are needed in the proper order to reconstruct
    it by calling [load] below. *)
...
(** Converting to the standard representation (see module [Std]).  The
   cross-module invariants are [Std.freeze (Opt.thaw d) = d] and
   [Opt.thaw (Std.freeze d) = d] *)
