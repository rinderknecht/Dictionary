(* This module exports two versions of the same abstract data type: a
   ...
   the right if strictly greater, and to the middle if equal.
   ...
   insertions, whilst the latter offers path compression (hence faster
   searches), but no insertions. Conversions between the two
...
(* The two implementations of ternary search trees *)
...
= struct
  (* A dictionary of type [t] is implemented as a ternary search
     tree.

       * [Nil] denotes an empty dictionary.
       ...
       * [Leg (c,l,m,r)] ("character, lower, equal, greater") is tree
         ...
         subtree is [m] and right subtree is [r]:
           ...
           - the subtree [l] denotes the dictionary containing words
             starting with a letter lower than [c], in alphabetic
             order;

           - the subtree [m] denotes a dictionary containing words
             such that prefixing them with the character [c], they
             belong to the dictionary denoted by [Leg (c,l,m,r)];

           - the subtree [r] denotes a dictionary holding words
...
and Opt : sig
  ...
  (* To save some memory, we can add a couple of value constructors
     ...
     process. The structural constraints are the following:
       ...
       * The constructors [Def] and [Pre] can only occur as the
         second argument of [Cmp] or the third of [Leg];

       * The unary constructor [Nil] can only occur as the second or
         fourth argument of [Leg]; it is also best, although not
         mandatory, to use [Cmp] instead if [Nil] occurs in both
         positions;

       * the first argument of [Cmp] should not be [""];

       * the second argument of [Pre] must either be [Leg] or [Cmp].

     The value [Def d] ("definition") is a tree containing only the
     definition [d], that is, it represents the mapping from the empty
     word to [d]. In fact, [Def d] is equivalent to [Pre(d,Nil)].

     The value [Cmp (p,s)] ("compressed path") is a tree containing
     words with the maximum prefix [p] in common, and whose suffixes
     are in the subtree [s], that is, it denotes a dictionary whose
     entries start with [p] and whose definitions are in [s]. It can
     be understood as the compression of a linear path in the tree, so
     [Comp(p,s)] is the same as [Leg(p.[0], Nil, Leg(p.[1], Nil, ...,
     Leg(p.[n], Nil, s, Nil), Nil)...), Nil)], where [n =
     String.length p]. *)
